#!/usr/bin/env python
"""Score GSE164789 scRNA-seq samples for LUAD niche signatures."""
...
from luad_niche.tenx import discover_10x_samples, read_10x_selected_genes_with_totals  # noqa: E402


STAGE_ORDER = ["Adjacent", "Tumor"]
REFINED_PANEL_NAMES = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=PROJECT_ROOT / "data" / "interim" / "GSE164789" / "raw_10x",
    )
    parser.add_argument(
        "--markers",
        type=Path,
...
def assign_macrophage_states(table: pd.DataFrame, min_score: float, min_margin: float) -> pd.DataFrame:
    ...
    result = table.copy()
    subtype = assign_dominant_labels(result, macrophage_columns, min_score=min_score, min_margin=min_margin)
...
def summarize_scores(table: pd.DataFrame, score_columns: list[str]) -> pd.DataFrame:
    records = []
    contexts = {
...
def plot_refined_scores(stage_summary: pd.DataFrame, output: Path) -> None:
    ...
    order = [stage for stage in STAGE_ORDER if stage in pivot.index]
    pivot = pivot.loc[order]
    ax = pivot.plot(kind="line", marker="o", figsize=(7, 4.8))
    ax.set_ylabel("Mean normalized score")
    ax.set_xlabel("")
    ax.set_title("GSE164789 refined discovery-signature scores")
    ax.legend(frameon=False, fontsize=8, bbox_to_anchor=(1.02, 1), loc="upper left")
    ax.figure.tight_layout()
    output.parent.mkdir(parents=True, exist_ok=True)
    ax.figure.savefig(output, dpi=220)
...
def main() -> int:
    ...
    samples = list(discover_10x_samples(args.input_dir, require_tissue_positions=False).values())
    samples = sorted(samples, key=lambda sample: sample.sample_name)
    if args.limit_samples:
        samples = samples[: args.limit_samples]
    if not samples:
        raise SystemExit(f"No expression-only 10x samples found under {args.input_dir}")
    ...
    for sample in samples:
        ...
        table = assign_macrophage_states(table, min_score=args.class_min_score, min_margin=args.class_min_margin / 2)
        ...
        score_sample_tables.append(summarize_scores(table, present_key_scores))
        print(
            f"{sample_accession}: stage={meta['stage']}; "
            f"cells={len(table)}; classes={table['broad_class'].value_counts().to_dict()}"
        )

    cells = pd.concat(compact_tables, ignore_index=True)
    cell_output = args.table_dir / "gse164789_scrna_cell_state_assignments.csv"
    ...
    outputs = {
        "gse164789_scrna_broad_class_sample_summary.csv": broad_sample,
        "gse164789_scrna_broad_class_stage_summary.csv": broad_stage,
        "gse164789_scrna_epithelial_state_sample_summary.csv": epithelial_sample,
        "gse164789_scrna_epithelial_state_stage_summary.csv": epithelial_stage,
        "gse164789_scrna_macrophage_state_sample_summary.csv": macrophage_sample,
        "gse164789_scrna_macrophage_state_stage_summary.csv": macrophage_stage,
        "gse164789_scrna_refined_signature_sample_summary.csv": score_sample,
        "gse164789_scrna_refined_signature_stage_summary.csv": score_stage,
    }
    for filename, frame in outputs.items():
        frame.to_csv(args.table_dir / filename, index=False, encoding="utf-8-sig")

    genes_used_output = args.table_dir / "gse164789_scrna_genes_used.json"
    ...
    plot_stage_fractions(
        broad_stage,
        "broad_class",
        args.figure_dir / "gse164789_scrna_broad_class_stage_fractions.png",
        "GSE164789 broad marker-score classes",
    )
    plot_stage_fractions(
        epithelial_stage,
        "epithelial_state",
        args.figure_dir / "gse164789_scrna_epithelial_state_stage_fractions.png",
        "GSE164789 epithelial marker-score states",
    )
    plot_stage_fractions(
        macrophage_stage,
        "macrophage_state",
        args.figure_dir / "gse164789_scrna_macrophage_state_stage_fractions.png",
        "GSE164789 macrophage marker-score states",
    )
    plot_refined_scores(score_stage, args.figure_dir / "gse164789_scrna_refined_signature_stage_scores.png")
